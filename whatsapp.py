...
def get_seller_team_recipients(seller_team_recipients):
    recipients = set()
    if seller_team_recipients:
        if isinstance(seller_team_recipients, str):
            ...
            # Handle list input
            for phone in seller_team_recipients:
                formatted_seller = format_phone_number(phone.strip())
                if formatted_seller:
                    recipients.add(formatted_seller)
    return recipients
...
if st.session_state["auto_check_enabled"]:
    ...
    if datetime.now() >= next_check_time:
        check_and_process_emails_automatically(st.session_state["email_search_query"])

# Start the scheduler in a separate thread
if not st.session_state.get("scheduler_started", False):
    schedule.every(st.session_state["check_interval_minutes"]).minutes.do(
        check_and_process_emails_automatically, subject_query=st.session_state["email_search_query"]
    )
