# This code provides a Streamlit-based dashboard for managing purchase orders (POs): it automates the extraction of order details from email PDFs, stores them in a PostgreSQL database, 
# and enables sending WhatsApp notifications to the seller team for pending orders, supporting both automated and manual order entry workflows
...
def send_whatsapp_message(message, recipient_numbers):  # Modified function
    ...
    for recipient in recipient_numbers:
        try:
            ...
            status_container.success(f"Message sent to {recipient} successfully!") # Per message status
        except Exception as e:
            ...
            if st.form_submit_button("Submit & Notify"):
                ...
                if not all(required_fields):
                    ...
                    if conn:
                        try:
                            if store_order(conn, order_details):
                                st.success("Order submitted and stored in database")
                                send_whatsapp_from_db()
                            else:
                                st.error("Failed to store order in database.")
                        except Exception as e:
                            st.error(f"Error storing order: {e}")
                        finally:
                            conn.close()
                    else:
