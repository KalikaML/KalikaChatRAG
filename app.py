...
import pandas as pd
import time
from scheduler import check_new_files_and_update_index  # Import the function you create
...
def main_app():
    with st.sidebar:
        ...
        if st.button("New Chat"):
            ...
            st.rerun()
        if 'new_file_count' not in st.session_state:
            st.session_state['new_file_count'] = 0

        if st.button("Refresh Data"):
            with st.spinner("Checking for new files..."):
                count = check_new_files_and_update_index()  # Call scheduler function
            st.session_state['new_file_count'] = count
            st.write(f"New files arrived: {st.session_state['new_file_count']}")
